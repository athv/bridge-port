...
Home.prototype.get_data_from_server = function(callback){
    ...
    var dow = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'][day]
    cur_date = String(new Date().getDate())+'-'+ String(new Date().getMonth()+1) + '-' + String(new Date().getFullYear())
    var pass_data = {
        'id': fuid,
        'date': cur_date,
        'day': dow
...
Home.prototype.create_base_template = function(){
    htmlstr =
        '<div id="subject-data" class="card huge">'+
            ...
            '</div>'+
            '<textarea id="notes-text" rows="15" placeholder="Enter your notes here"></textarea>'+
        '</div>'
...
AboutView.prototype.populate_about_content = function(){
    ...
    htmlstr +=
        '<div id="team-image-container">'+
            '<img id="team-image" src="static/core/images/logo.png" alt=" Team">'+
        '</div>'+
        ...
        '<div class="team-data-content">'+
            '<br>'+
            '2021MT93268 : Ancy Andrews <br>'+
            '<br>'+
            '2021MT93044 : Athulya V <br>'+
            '<br>'+
            '2021MT93339 : Harikrishnan B <br>'+
            '<br>'+
            '2021MT93308 : Rajesh Kumar T <br>'+
            '<br>'+
        '</div>'
...
function onSignIn(googleUser) {
    ...
    $.post(server_address+'/signin/', { data:JSON.stringify(user_data) } ,  function(data){
        ...
        if(data['exists'] == true){
            ...
            $('#popup-welcome-message').text('Hello '+user_data['name']+' , welcome to ClassBuddy!')
...
$('div').not('.leftpane').click(function(){
    cur_state = $('#hamburger-menu-button').css('display');  
    if(cur_state == 'block'){
        nav = $($('.leftpane')[0]);
...
$('div').not('.rightpane').click(function(){
    cur_state = $('#upcoming-menu-button').css('display');  
    if(cur_state == 'block'){
        nav = $($('.rightpane')[0]);
        nav.css('display', 'none');
        nav.css('width', '20vw');
